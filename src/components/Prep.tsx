...
import { calcBox, fmtQty, nf } from '@/lib/calc';
...
export function Prep() {
  ...
  const c: Ctx = { sel, picks, now, mark, view, open };
...
interface Ctx { sel: Sel; picks: readonly Pick[]; now: Cost; mark: (kit: Kit, p: Protein) => void; view: View | null; open: (kit: Kit, p: Protein) => void }
  ...
  return (
    <motion.article initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={spring} style={{ '--hue': kit.hue } as React.CSSProperties}
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="grid grid-cols-[1fr_auto] gap-x-3 gap-y-0.5 text-[13px]">
          {kit.base && <><span>{byId(BASES, kit.base).name}</span><span className="font-mono">{t.base.portion}</span></>}
