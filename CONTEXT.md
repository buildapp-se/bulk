...
- **Timers (2026-09-24).** Bara steg med `Step.what` (protein, kolhydrat, grönt, sous vide, form, gryta, tomatbasen) får timer. En klocka är `{ end }` när den går och `{ left }` när den är pausad (`src/lib/clock.ts`), så paus och ± minuter behöver inga extra fält. Dockan och larmet ligger i headern och ringer därför på alla sidor. Larm: Web Audio var 2:a sekund plus vibration tills man kvitterar, en avisering per klar klocka. Kvittera = klockan bort och steget avbockat. Ljudet låses upp av första tryck på sidan (iOS kräver gest).
